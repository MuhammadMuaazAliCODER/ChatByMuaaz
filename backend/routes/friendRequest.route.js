...
// checkFriendLimit runs first — blocks if either user is at their plan limit.
// ─────────────────────────────────────────────────────────────────────────────
const respondRequest = async (req, res) => {
  try {
    ...
    if (action === 'accepted') {
      ...
      if (!alreadyFriends) {
        await Friend.create({ user1: request.from, user2: request.to });
      }

      // 2️⃣ Create direct chat if not exists
      ...
      // 3️⃣ Increment friendsCount for both users
      if (req.countFriend) {
        await req.countFriend();
      }
    }
...
router.post('/respond', auth, checkFriendLimit, respondRequest); // ✅ limit enforced
